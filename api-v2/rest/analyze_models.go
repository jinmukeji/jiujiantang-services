...
type AnalysisReportData struct {
	...
	AnalysisResultError bool            `json:"analysis_report_result_error"`
}
...
type Content struct {
	...
	RiskEstimate                        []RiskEstimate    `json:"risk_estimate"`
	RiskEstimateTips                    []string          `json:"risk_estimate_tips"`
	TreatmentAdvices                    []TreatmentAdvice `json:"treatment_advices"`
	SyndromeDifferentiationConstitution string            `json:"syndrome_differentiation_constitution"`

	CreatedAt time.Time `json:"created_at"`
}

// TreatmentAdvice 治疗建议
type TreatmentAdvice struct {
...
type RiskEstimate struct {
	DiseaseName string `json:"disease_name"` // 18种疾病
	Status      int    `json:"status"`       // 3重风险、2中风向、1轻风险、0正常
}
