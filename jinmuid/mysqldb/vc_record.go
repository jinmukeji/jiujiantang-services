...
func (db *DbClient) SearchVcRecordCountsIn24hours(ctx context.Context, sendTo string) (int, error) {
	var count int
	err := db.GetDB(ctx).Model(&VcRecord{}).Where("send_to = ? and created_at > DATE_SUB(CURDATE(), INTERVAL 8 HOUR)", sendTo).Count(&count).Error
	if err != nil {
		return 0, err
	}
	return count, nil
...
func (db *DbClient) SearchVcRecordCountsIn1Minute(ctx context.Context, sendTo string) (int, error) {
	var count int
	err := db.GetDB(ctx).Model(&VcRecord{}).Where("send_to = ? and created_at > DATE_SUB(NOW(), INTERVAL 1 minute) and has_used = ? ", sendTo, false).Count(&count).Error
	if err != nil {
		return 0, err
	}
	return count, nil
...
func (db *DbClient) SearchVcRecordEarliestTimeIn1Minute(ctx context.Context, sendTo string) (*VcRecord, error) {
	var record VcRecord
	err := db.GetDB(ctx).Raw(`SELECT created_at FROM verification_code WHERE send_to = ?
...
func (db *DbClient) FindVcRecord(ctx context.Context, sn string, vc string, sendTo string, usage Usage) (*VcRecord, error) {
	var record VcRecord
	err := db.GetDB(ctx).Raw("SELECT record_id,expired_at,has_used FROM verification_code WHERE sn = ? AND code = ? AND send_to = ? AND has_used = ? AND `usage` = ? ORDER BY created_at LIMIT 1", sn, vc, sendTo, false, usage).Scan(&record).Error
	return &record, err
}
...
func (db *DbClient) HasSnExpired(ctx context.Context, sn string, vc string) (bool, error) {
	var count int
	err := db.GetDB(ctx).Model(&VcRecord{}).Where("sn = ? and code = ? and expired > NOW() and has_used = ? ", sn, vc, false).Count(&count).Error
	if err != nil {
		return false, err
	}
	if count != 0 {
...
func (db *DbClient) ModifyVcRecordStatus(ctx context.Context, recordID int32) error {
	return db.GetDB(ctx).Model(&VcRecord{}).Where("record_id = ?", recordID).Updates(map[string]interface{}{
		"has_used": true,
	}).Error
}
...
func (db *DbClient) VerifyMVC(ctx context.Context, sn string, vc string, sendTo string, nationCode string) (bool, error) {
	var count int
	err := db.GetDB(ctx).Raw(`SELECT count(record_id) FROM verification_code WHERE sn = ?
...
func (db *DbClient) SearchVcRecord(ctx context.Context, sn string, vc string, sendTo string, nationCode string) (*VcRecord, error) {
	var record VcRecord
	err := db.GetDB(ctx).Raw(`SELECT record_id,expired_at,has_used FROM verification_code WHERE sn = ?
        AND code = ? AND send_to = ? AND expired_at > NOW() AND has_used = ? AND nation_code = ?
...
func (db *DbClient) FindLatestVcRecord(ctx context.Context, sendTo string, usage Usage) (*VcRecord, error) {
	var record VcRecord
	err := db.GetDB(ctx).Raw("SELECT sn, code, record_id, expired_at FROM verification_code WHERE `usage` = ? AND send_to = ? AND has_used = ? AND created_at > DATE_SUB(CURDATE(), INTERVAL 8 HOUR) ORDER BY created_at DESC LIMIT 1", usage, sendTo, false).Scan(&record).Error
	return &record, err
}
...
func (db *DbClient) SearchSpecificVcRecordCountsIn24hours(ctx context.Context, sendTo string, usage Usage) (int, error) {
	var count int
	err := db.GetDB(ctx).Model(&VcRecord{}).Where("send_to = ? and `usage` = ? and created_at > DATE_SUB(CURDATE(), INTERVAL 8 HOUR)", sendTo, usage).Count(&count).Error
	if err != nil {
		return 0, err
	}
	return count, nil
...
func (db *DbClient) SearchSpecificVcRecordEarliestTimeIn24hours(ctx context.Context, sendTo string, usage Usage) (*VcRecord, error) {
	var record VcRecord
	err := db.GetDB(ctx).Raw("SELECT expired_at FROM verification_code WHERE send_to = ? AND `usage` = ? AND created_at > DATE_SUB(CURDATE(), INTERVAL 8 HOUR) ORDER BY created_at LIMIT 1", sendTo, usage).Scan(&record).Error
	if err != nil {
		return nil, err
	}
	return &record, nil
...
func (db *DbClient) SearchLatestPhoneVerificationCode(ctx context.Context, sendTo string, nationCode string) (string, error) {
	var record VcRecord
	err := db.GetDB(ctx).Raw(`SELECT code FROM verification_code WHERE send_to = ? AND nation_code = ? 
...
func (db *DbClient) SearchLatestEmailVerificationCode(ctx context.Context, sendTo string) (string, error) {
	var record VcRecord
	err := db.GetDB(ctx).Raw(`SELECT code FROM verification_code WHERE send_to = ? ORDER BY created_at DESC LIMIT 1`, sendTo).Scan(&record).Error
	if err != nil {
		return "", err
	}
	return record.Code, nil
...
func (db *DbClient) VerifyMVCBySecureEmail(ctx context.Context, sn string, vc string, email string) (*VcRecord, error) {
	var record VcRecord
	err := db.GetDB(ctx).Raw(`SELECT expired_at,has_used FROM verification_code 
...
func (db *DbClient) ModifyVcRecordStatusByEmail(ctx context.Context, email, verificationCode, serialNumber string) error {
	return db.GetDB(ctx).Model(&VcRecord{}).Where("send_to = ? and code = ? and sn = ?", email, verificationCode, serialNumber).Updates(map[string]interface{}{
		"has_used": true,
	}).Error
}
...
func (db *DbClient) SetVcAsUsed(ctx context.Context, sn string, vc string, sendTo string, nationCode string) error {
	return db.GetDB(ctx).Model(&VcRecord{}).Where("sn = ? and code = ? and send_to = ? and nation_code = ?", sn, vc, sendTo, nationCode).Updates(map[string]interface{}{
		"has_used": true,
	}).Error
}
...
func (db *DbClient) SearchVcRecordFrom1MinuteTo2Mintue(ctx context.Context, sendTo string, usage Usage) (*VcRecord, error) {
	var record VcRecord
	err := db.GetDB(ctx).Raw("SELECT code FROM verification_code WHERE send_to = ? AND created_at > DATE_SUB(NOW(), INTERVAL 2 minute) and created_at < DATE_SUB(NOW(), INTERVAL 1 minute) and `usage` = ? ORDER BY created_at LIMIT 1", sendTo, usage).Scan(&record).Error
	if err != nil {
		return nil, err
	}
	return &record, nil
