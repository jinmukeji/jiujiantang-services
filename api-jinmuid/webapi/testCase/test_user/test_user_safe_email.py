...
class UserSafeEmail(ApiTestCase):
    u'''用户设置安全邮箱'''
    ...
    def safe_email(self, verification_code, serial_number, email):
        data = {"verification_code": verification_code,
                "serial_number": serial_number,
                "email": email}
        url = self.host + 'user/' + str(Util().get_user_id()) + '/safe_email'
        LOG.info("请求url:%s" % url)
        res = requests.post(url=url, json=data, headers=Util().get_token())
        LOG.info("请求参数:%s" % data)
        return res.json()

    serial_number = Util().get_email_set_secure_email_serial_number()
    verification_code = Util().email_verification_code()

    def test_safe_email_correct_parameters(self):
        u"""正确参数"""
        LOG.info("------用户设置安全邮箱：start!---------")
        result = self.safe_email(self.verification_code, self.serial_number, data.email)
    ...
    def test_safe_email_mvc_null(self):
        u"""mvc为空"""
        LOG.info("------用户设置安全邮箱：start!---------")
        result = self.safe_email('', self.serial_number, data.email)
        LOG.info("获取测试结果：%s" % result)
    ...
    def test_safe_email_error_email(self):
        u"""email验证码错误"""
        LOG.info("------用户设置安全邮箱：start!---------")
        result = self.safe_email(self.verification_code, self.serial_number, '')
        LOG.info("获取测试结果：%s" % result)
        self.assertErrorResult(result, const.InvalidVcRecord)
        LOG.info("------pass!---------")
    ...
    def test_safe_email_error_email_format(self):
        u"""email格式错误"""
        LOG.info("------用户设置安全邮箱：start!---------")
        result = self.safe_email(self.verification_code, self.serial_number, 'cuimin')
        LOG.info("获取测试结果：%s" % result)
        self.assertErrorResult(result, const.InvalidVcRecord)
        LOG.info("------pass!---------")

    def test_safe_email_is_used(self):
        u"""邮箱已被绑定"""
        LOG.info("------用户设置安全邮箱：start!---------")
        result = self.safe_email(self.verification_code, self.serial_number, data.email)
        LOG.info("获取测试结果：%s" % result)
