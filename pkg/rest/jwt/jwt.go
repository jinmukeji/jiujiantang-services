...
var (
	...
	// 默认jwt的过期时间是7天
	defalutJwtDuration = time.Hour * 24 * 7
)
...
func (m *Middleware) BuildJwtToken(hmac *jwt.SigningMethodHMAC, clientID, zone, name, customizedCode string) (string, error, time.Time) {
	...
	authorization, err := token.SignedString([]byte(m.Config.JwtSignInKey))
	return authorization, err, expiredAt
}
