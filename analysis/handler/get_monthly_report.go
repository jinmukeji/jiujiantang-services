...
func convertAEInputKey(inputKeys []*proto.Lookup) []interface{} {
	iks := make([]interface{}, len(inputKeys))
	for idx, inputKey := range inputKeys {
		ik := make(map[string]interface{})
		ik["key"] = inputKey.Key
		ik["score"] = inputKey.Score
		iks[idx] = ik
	}
	return iks
}

func convertPhysicalDialecticsToAEInput(physicalDialectics []string) InputKeys {
	iks := make([]InputKey, len(physicalDialectics))
	for idx, inputKey := range physicalDialectics {
		ik := make(map[string]interface{})
		ik["key"] = inputKey
		iks[idx] = ik
	}
...
// GetMonthlyAnalyzeResult 得到分析报告的内容
func (j *AnalysisManagerService) GetMonthlyAnalyzeResult(ctx context.Context, req *proto.GetMonthlyAnalyzeResultRequest, resp *proto.GetMonthlyAnalyzeResultResponse) error {
	// 获得用户个人信息
	...
	// 加载月报的 preset
	err = j.biz.LoadPresets(j.presetsFilePath, biz.MonthlyReportModule)
	if err != nil {
		return NewError(ErrClientID, fmt.Errorf("failed to get preset by client_id %s: %s", clientID, err.Error()))
	}
	...
	if errRunEngine != nil {
		// 分析有错
		return NewError(ErrAEError, fmt.Errorf("error occurs when run monthly engine: %s", errRunEngine.Error()))
	}
	// 构建返回的内容
	...
	respModules, err := getModulesOfMonthlyReport(*newCtxData, displayOptions, userProfile.GetGender())
	if err != nil {
...
// InputKey 输入的Key
type InputKey map[string]interface{}

// InputKeys InputKey数组
type InputKeys []InputKey
