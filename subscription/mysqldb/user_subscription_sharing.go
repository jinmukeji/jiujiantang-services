...
func (db *DbClient) CreateUserSubscriptionSharing(ctx context.Context, UserSubscriptionSharing *UserSubscriptionSharing) error {
	return db.Create(UserSubscriptionSharing).Error
}
...
func (db *DbClient) GetUserCountOfSubscription(ctx context.Context, subscriptionID int32) (int32, error) {
	var count int
	err := db.Model(&UserSubscriptionSharing{}).Where("subscription_id = ? AND user_subscription_sharing.deleted_at is NULL", subscriptionID).Count(&count).Error
...
func (db *DbClient) DeleteSubscriptionUsers(ctx context.Context, userIDList []int32, subscriptionID int32) error {
	if err := db.Delete(UserSubscriptionSharing{}, "subscription_id = ? AND user_id IN (?)", subscriptionID, userIDList).Error; err != nil {
		return err
	}
	return nil
}
...
func (db *DbClient) CheckExistUserSubscriptionSharing(ctx context.Context, userID int32, subscriptionID int32) (bool, error) {
	var count int
	err := db.Model(&UserSubscriptionSharing{}).Where("user_id = ? and subscription_id = ?", userID, subscriptionID).Count(&count).Error
	return count >= 1, err
}
