...
// ClientAuth 客户端授权
func (j *JinmuHealth) ClientAuth(ctx context.Context, req *proto.ClientAuthRequest, resp *proto.ClientAuthResponse) error {
	client, err := j.datastore.FindClientByClientID(ctx, req.ClientId)
	if err != nil {
		return NewError(ErrDatabase, fmt.Errorf("failed to find client %s: %s", req.ClientId, err.Error()))
	}
